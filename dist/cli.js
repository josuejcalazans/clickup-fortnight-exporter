...
import { DEFAULT_HOURLY_RATE } from "@src/config";
...
import { getQuinzenaRange } from "@utils/time";
export async function runCli() {
    ...
    try {
        ...
        console.log(` - ${defaultStartDate.toISOString().slice(0, 10)} até ${defaultEndDate
            .toISOString()
            .slice(0, 10)}`);
        console.log();
        ...
        if (useCustom.toLowerCase() === "s") {
            console.log();
            console.log("Informe a DATA INICIAL (dia/mês/ano). Deixe em branco para usar o início padrão da quinzena.");
            const defaultStartStr = `${String(defaultStartDate.getDate()).padStart(2, "0")}/${String(defaultStartDate.getMonth() + 1).padStart(2, "0")}/${defaultStartDate.getFullYear()}`;
            const startDateInput = (await rl.question(`Data inicial (DD/MM/AAAA) [${defaultStartStr}]: `)).trim();
            console.log();
            console.log("Informe a DATA FINAL (dia/mês/ano). Deixe em branco para usar o fim padrão da quinzena.");
            const defaultEndStr = `${String(defaultEndDate.getDate()).padStart(2, "0")}/${String(defaultEndDate.getMonth() + 1).padStart(2, "0")}/${defaultEndDate.getFullYear()}`;
            const endDateInput = (await rl.question(`Data final (DD/MM/AAAA) [${defaultEndStr}]: `)).trim();
            let startDate;
            let endDate;
            if (!startDateInput) {
        ...
        console.log();
        const hourlyRateDefaultText = DEFAULT_HOURLY_RATE > 0 ? ` [padrão: ${DEFAULT_HOURLY_RATE.toFixed(2)} USD]` : "";
        const hourlyRateStr = (await rl.question(`Valor da sua hora em USD${hourlyRateDefaultText}: `)).trim();
        ...
        if (!hourlyRate || hourlyRate <= 0) {
            throw new Error("Nenhum valor de hora válido foi definido (nem no .env nem via CLI).");
        }
        console.log();
        ...
        console.log(`Total de horas: ${result.totalHoursInt}h ${result.totalMinutesRemainder}m (${result.totalHoursDecimal.toFixed(2)})`);
